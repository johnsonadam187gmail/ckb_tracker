...
def check_password():
    ...
    def password_entered():
        ...
        if st.session_state["username"] == "admin" and st.session_state["password"] == "ckb2026":
...
if check_password():
    ...
    st.set_page_config(page_title="Management Console", layout="wide")
    ...
    with tab_user:
        ...
        try:
            ...
            if u_res.status_code == 200:
                ...
                if not all_users:
                    st.info("No members found in the database. Add them via the main dashboard.")
                else:
                    ...
                    with col_search:
                        search_query = st.text_input("🔍 Search Members", placeholder="Filter by name, rank, or email...")
                    
                    # Filter logic for the table
                    if search_query:
                        mask = df_u.apply(lambda row: row.astype(str).str.contains(search_query, case=False).any(), axis=1)
                        df_u = df_u[mask]
                    ...
                    # Display Table
                    st.dataframe(
                        df_u[["first_name", "last_name", "rank", "email", "created_date"]], 
                        width='stretch', 
                        hide_index=True
                    )
                    ...
                    st.caption("Updating a member will archive their current record and create a new version to maintain history.")
                    
                    # Map names for the selection process
                    user_map = {f"{u['first_name']} {u['last_name']} ({u['rank']})": u for u in all_users}
                    ...
                    if selected_user_key != "-- Select Member --":
                        ...
                        with st.form("edit_member_form"):
                            ...
                            up_first = c1.text_input("First Name", value=member['first_name'])
                            up_last = c2.text_input("Last Name", value=member['last_name'])
                            ...
                            up_rank = c1.selectbox("Current Rank", options=ranks, index=rank_idx)
                            up_email = c2.text_input("Email Address", value=member['email'])
                            
                            up_nick = st.text_input("Nicknames (Optional)", value=member.get('nicknames', ''))
                            
                            # Form Submission
                            submit_update = st.form_submit_button("Save Changes & Archive History")
            ...
            with st.expander("➕ Add New Class to Timetable"):
                if not gym_opts or not type_opts:
                    ...
                    with st.form("new_class_form"):
                        ...
                        c_gym = st.selectbox("Gym Location", options=list(gym_opts.keys()))
                        c_typ = st.selectbox("Class Type", options=list(type_opts.keys()))
                        
                        if st.form_submit_button("Create Class"):
                            handle_request("POST", "classes", {
                                "class_name": c_name, "day": c_day, "time": c_time,
                                "weighting": c_wght, "gym_id": gym_opts[c_gym],
                                "class_type_id": type_opts[c_typ]
                            })

            if classes:
                st.subheader("Current Active Schedule")
                st.dataframe(pd.DataFrame(classes)[["class_name", "day", "time", "weighting"]], width='stretch', hide_index=True)
        except Exception as e:
    ...
    with tab_gym_types:
        ...
        with col_g:
            ...
            if g_res.status_code == 200:
                st.dataframe(pd.DataFrame(g_res.json()), width='stretch', hide_index=True)
        ...
        with col_t:
            ...
            if t_res.status_code == 200:
                st.dataframe(pd.DataFrame(t_res.json()), width='stretch', hide_index=True)
    ...
    with tab_terms:
        ...
        with st.form("term_form"):
            ...
            if st.form_submit_button("Create Term"):
                handle_request("POST", "terms", {"term_name": term_n, "start_date": str(s_date), "end_date": str(e_date)})
        
        term_res = requests.get(f"{BASE_URL}/terms/")
        if term_res.status_code == 200:
            st.dataframe(pd.DataFrame(term_res.json()), width='stretch', hide_index=True)
    ...
    with tab_targets:
        ...
        if terms_res.status_code == 200 and terms_res.json():
            ...
            with st.form("target_form"):
                ...
                t_rank = st.selectbox("Target Rank", ["White", "Blue", "Purple", "Brown", "Black"])
                t_val = st.number_input("Hours Required", min_value=0.0)
                if st.form_submit_button("Set Target"):
                    handle_request("POST", "term-targets", {"term_id": term_map[t_term], "rank": t_rank, "target": t_val})
            
            # Display existing
            targ_res = requests.get(f"{BASE_URL}/term-targets/")
            if targ_res.status_code == 200:
                st.dataframe(pd.DataFrame(targ_res.json()), width='stretch', hide_index=True)
        else:
