...
selected_student_name = st.sidebar.selectbox("Select Student", options=list(user_map.keys()))
...
attendance_data = []
try:
    att_res = requests.get(f"{BASE_URL}/attendance/user/{user_uuid}?start={start_dt}&end={end_dt}")
...
if attendance_data:
    ...
    weight_col = next((c for c in ['class_weighting', 'weighting', 'weight'] if c in df_att.columns), None)
    
    if weight_col:
        # Convert to numeric to ensure we can sum safely
        df_att[weight_col] = pd.to_numeric(df_att[weight_col], errors='coerce').fillna(0)
...
with chart_col:
    ...
    if selected_term_name != "All Time":
        relevant_target = next(
            (t for t in targets if t['term_id'] == term['id'] and t['rank'] == user['rank']), 
            None
        )
        if relevant_target:
...
if attendance_data:
    ...
    time_col = next((c for c in ['check_in_time', 'timestamp', 'created_at'] if c in df_att.columns), None)
    name_col = next((c for c in ['class_name', 'name', 'label'] if c in df_att.columns), None)
    weight_col = next((c for c in ['class_weighting', 'weighting', 'weight'] if c in df_att.columns), None)
    ...
    if time_col:
        ...
        with col_left:
            ...
            fig_line = px.area(daily_points, x='date', y='cumulative', 
    ...
    with col_right:
        ...
        if name_col:
            ...
            fig_pie = px.pie(df_att, names=name_col, values=weight_col, 
                             hole=0.4, title="Points by Class Type")
    ...
    display_cols = [c for c in [time_col, name_col, 'day', weight_col] if c in df_att.columns]
    df_display = df_att[display_cols].copy()
    
    # Beautify the timestamp if it exists
    if time_col in df_display.columns:
        df_display[time_col] = pd.to_datetime(df_display[time_col]).dt.strftime('%b %d, %Y - %H:%M')
